...
# PURPOSE
# -----------------------------------------------------------------------------
#   Stage-1 Flibusta extraction by AUTHOR NAME: for each name argument,
#   resolve the matching authors in the 'flibusta' catalog
#   (mlauthorname.FullName LIKE '%name%'), collect the FileNumbers of
#   their fb2 books (mlbook JOIN mlauthor), and extract every number's
#   member from the f.fb2-* range archives into FB2_OUTPUT_DIR - the
#   whole author shelf in one command.
#
...
# USAGE
# -----------------------------------------------------------------------------
#   ./bin/flibusta/extract_author_flibusta.sh [options] NAME...
#   ./bin/flibusta/extract_author_flibusta.sh [options] --from-file LIST
#
#   Options:
#       -f, --from-file LIST    author names from LIST (one per line;
...
#           --limit N           max books per author (0 = no limit)
#                               [default: 0]
...
# ALGORITHM
# -----------------------------------------------------------------------------
#   Per name: flb_sql_like_escape -> authors query (mlauthorname.FullName
#   LIKE '%name%'; no match = per-name failure) -> books query (JOIN
#   mlauthor, fb2 bare-number filenames only, lowest-authorid dedupe via
#   seen[], optional --limit) -> per number: validate -> flb_find_archive
...
readonly CLI_INVOCATION="bin/flibusta/extract_author_flibusta.sh"
...
print_help() {
    cat >&2 <<'EOF'
Usage: extract_author_flibusta.sh [options] NAME... | --from-file LIST

Stage-1 Flibusta extraction by AUTHOR NAME: resolve each name against
mlauthorname.FullName (substring match), collect the FileNumbers of the
authors' fb2 books, and extract every number's member from the f.fb2-*
...
extract_author_flibusta.sh (this tool), extract_series_flibusta.sh
(by series name).

Options:
  -f, --from-file LIST      author names from LIST, one per line (blank
  ...
  -o, --output-dir DIR      extraction target [default: /mnt/c/Backup_Go7/ToLoad]
      --db NAME             catalog database [default: flibusta]
      --limit N             max books per author (0 = no limit) [default: 0]
      --force               re-extract even if the output already exists
  -n, --dry-run             resolve and list; extract nothing
  -d, --debug               verbose diagnostics on stderr
  -h, --help                show this help
...
# --- resolution: name -> author rows / book numbers -------------------------------
# authors_for_name NAME -> "authorid<TAB>FullName" rows on stdout.
# The pattern is escaped by flb_sql_like_escape (quotes/backslash/wildcards).
authors_for_name() { # $1 = raw author name
    local raw="$1" esc
    esc="$(flb_sql_like_escape "$raw")"
    local sql
    sql="SELECT an.authorid, an.FullName
FROM mlauthorname an
WHERE an.FullName LIKE '%${esc}%'
ORDER BY an.authorid;"
    db_run_sql "$sql" "$FLIBUSTA_DB" 2>/dev/null || return 1
}

# numbers_for_authors AUTHOR_IDS... -> "filename<TAB>authorid<TAB>FullName"
# rows on stdout, fb2 bare-number filenames only.  A book with several
# matched authors appears once per author; the caller dedupes by number.
numbers_for_authors() { # $@ = authorids
    local esc_ids="" id
    for id in "$@"; do
    ...
    local sql
    sql="SELECT b.filename, a.authorid, an.FullName
FROM mlbook b
JOIN mlauthor a ON a.bookid = b.bookid
JOIN mlauthorname an ON an.authorid = a.authorid
WHERE a.authorid IN ($esc_ids) AND b.filename REGEXP '^[0-9]+[[:space:]]*$'
ORDER BY an.FullName, CAST(b.filename AS UNSIGNED)
$limit_clause;"
    db_run_sql "$sql" "$FLIBUSTA_DB" 2>/dev/null || return 1
}
...
declare -A seen_number=()   # global dedupe: the same book under two matched authors

for name in "${names[@]}"; do
    author_rows="$(authors_for_name "$name")" || {
        failures+=("$name: catalog query failed")
        log_warn "$name: catalog query failed"
        fail_count=$((fail_count + 1))
        continue
    }
    if [[ -z "$author_rows" ]]; then
        failures+=("$name: no author matched (mlauthorname.FullName)")
        log_warn "$name: no author matched"
        fail_count=$((fail_count + 1))
        continue
    fi

    # collect author ids + log the matches
    declare -a author_ids=()
    while IFS=$'\t' read -r aid afull; do
        author_ids+=("$aid")
        debug "$name: author $aid = $afull"
    done <<< "$author_rows"
    log_info "$name: matched ${#author_ids[@]} author(s)"

    book_rows="$(numbers_for_authors "${author_ids[@]}")" || {
    ...
    if [[ -z "$book_rows" ]]; then
        failures+=("$name: matched author(s) have no fb2 books on file")
        log_warn "$name: matched author(s) have no fb2 books on file"
        fail_count=$((fail_count + 1))
        continue
    fi
    ...
    name_failed=0
    while IFS=$'\t' read -r file_number aid afull; do
        [[ -n "${seen_number[$file_number]:-}" ]] && continue
        seen_number["$file_number"]=1

        if ! flb_validate_number "$file_number"; then
            failures+=("$file_number: invalid FileNumber for author '$afull'")
            log_warn "$file_number: invalid FileNumber for author '$afull'"
            name_failed=1
            continue
        fi
        ...
        if [[ -s "$output_file" ]] && (( ! FORCE )); then
            debug "$file_number [$afull]: output exists, skipping ($output_file)"
            ok_count=$((ok_count + 1))
        ...
        if flb_find_archive "$file_number" fb2; then
            ...
            failures+=("$file_number: no fb2 archive contains the number ($afull)")
            log_warn "$file_number: no fb2 archive contains the number ($afull)"
            name_failed=1
            continue
        fi
        ...
        if flb_resolve_member "$archive" "$file_number" fb2; then
            ...
            failures+=("$file_number: member not present in $(basename "$archive") ($afull)")
            log_warn "$file_number: member not present in $(basename "$archive") ($afull)"
            name_failed=1
            continue
        fi
        ...
        if (( is_batch )); then
            log_info "$file_number [$afull]: $(basename "$archive") -> $member"
        else
            log_info "author : $afull (authorid $aid)"
        ...
        if flb_extract "$archive" "$member" "$output_file"; then
            ok_count=$((ok_count + 1))
            (( is_batch )) && log_info "$file_number [$afull]: done -> $output_file"
        else
