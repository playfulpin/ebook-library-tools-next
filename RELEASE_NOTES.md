...
- `bin/check_layers.sh` **1.0.0** — layer-boundary gate (Follow-It §4): `lib/` code lines must be domain-free and every `bin/` tool may source only from `lib/`; fails CI on violation. The AWK parity reference is a documented, deliberate exemption (its variable vocabulary IS the domain by design)
